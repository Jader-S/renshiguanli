...
<form id="form1" name="form1" method="post" action="">
  搜索: 员工编号：<input name="yuangongbianhao" type="text" id="yuangongbianhao" style='border:solid 1px #000000; color:#666666;width:80px' /> 员工姓名：<input name="yuangongxingming" type="text" id="yuangongxingming" style='border:solid 1px #000000; color:#666666;width:80px' /> 所属部门：<input name="suoshubumen" type="text" id="suoshubumen" style='border:solid 1px #000000; color:#666666;width:80px' /> 年份：<select name='nianfen' id='nianfen' style='border:solid 1px #000000; color:#666666;'><option value="">所有</option><?php for($nfn=date('Y');$nfn>date('Y')-5;$nfn--){?><option value='<?php echo $nfn?>'><?php echo $nfn?></option><?php } ?></select> 月份：<select name='yuefen' id='yuefen' style='border:solid 1px #000000; color:#666666;'><option value="">所有</option><?php for($nfn=1;$nfn<=12;$nfn++){?><option value='<?php echo $nfn?>'><?php echo $nfn?></option><?php } ?></select>
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">  
  ...
  <?php 
    $sql="select * from kaoqinxinxi where yuangongbianhao= '".$_SESSION["username"]."'";
	...
	for($i=$pagecurrent*$pagelarge-$pagelarge;$i<$ddddd;$i++)
{
  ...
  <tr>
    ...
    <td width="120" align="center"><a href="del.php?id=<?php echo mysql_result($query,$i,"id");?>&tablename=kaoqinxinxi" onclick="return confirm('真的要删除？')">删除</a> <a href="kaoqinxinxi_updt.php?id=<?php echo mysql_result($query,$i,"id");?>">修改</a> <a href="kaoqinxinxi_detail.php?id=<?php echo mysql_result($query,$i,"id");?>">详细</a> </td>
  </tr>
  	<?php
	}
}
...
<p>以上数据共<?php echo $rowscount;?>条,<!--yoxugonxgzitoxnxgjxi--> <!--youzuiping3-->
  <input type="button" name="Submit2" onclick="javascript:window.print();" value="打印本页" style='border:solid 1px #000000; color:#666666' /> <input type="button" name="Submit3" onclick="javascript:location.href='kaoqinxinxi_listxls.php';" value="导出EXCEL" style='border:solid 1px #000000; color:#666666' />
</p>
<p align="center"><a href="kaoqinxinxi_list.php?pagecurrent=1">首页</a>, <a href="kaoqinxinxi_list.php?pagecurrent=<?php echo $pagecurrent-1;?>">前一页</a> ,<a href="kaoqinxinxi_list.php?pagecurrent=<?php echo $pagecurrent+1;?>">后一页</a>, <a href="kaoqinxinxi_list.php?pagecurrent=<?php echo $pagecount;?>">末页</a>, 当前第<?php echo $pagecurrent;?>页,共<?php echo $pagecount;?>页 </p>
</body>
</html>
